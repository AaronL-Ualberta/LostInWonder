class ArtifactText extends EngineInstance {
  onEngineCreate() {
    ...
    this.caveevidence = [
      ...
      new DialogueLine(
        "These are drag marks from something that didn't want to leave its home."
      ),
    ...
    this.bg_box = $engine.createManagedRenderable(
      this,
      new PIXI.Sprite($engine.getTexture("msgbox"))
    );
  ...
  step() {
    if (this.fade) {
      ...
      this.dialogue.alpha = EngineUtils.interpolate(
        this.fade_value,
        1,
        0,
  ...
  collected() {
    ...
    if (RoomManager.currentRoom().name === "Tutorial") {
      this.dialogue_text.text =
        this.jungleevidence[
          EngineUtils.irandomRange(0, this.jungleevidence.length - 1)
        ].text;
    } else if (RoomManager.currentRoom().name === "Level1") {
      this.dialogue_text.text =
        this.caveevidence[
          EngineUtils.irandomRange(0, this.caveevidence.length - 1)
        ].text;
    } else if (RoomManager.currentRoom().name === "Level2") {
      this.dialogue_text.text =
        this.treesevidence[
          EngineUtils.irandomRange(0, this.treesevidence.length - 1)
        ].text;
    } else if (RoomManager.currentRoom().name === "Level3") {
      this.dialogue_text.text =
        this.swampevidence[
          EngineUtils.irandomRange(0, this.swampevidence.length - 1)
        ].text;
    } else {
