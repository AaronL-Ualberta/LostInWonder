class PlayerInstance extends EngineInstance {
  onEngineCreate() {
    ...
    this.mainHitbox = new Hitbox(
      this,
      new RectangleHitbox(-20, 34 * -2, 20, 0)
    );
    ...
    this.animation = $engine.createRenderable(
      this,
      new PIXI.extras.AnimatedSprite(this.animation_standing),
      false
    );
    // this.spr_scale = 1.2;
    this.spr_scale = 2;
  ...
  step() {
    ...
    if (wand_piece !== undefined) {
      ...
      if (this.spells_learned === 4) {
        this.levelHandler.spellWheel_sprite.texture =
          $engine.getTexture("spellwheel");
      } else {
        this.levelHandler.spellWheel_sprite.texture = $engine.getTexture(
          "spellwheel" + this.spells_learned
        );
      }
    ...
    if (IN.mouseCheckPressed(0) && this.spells_learned > 0) {
      if (this.current_spell === SPELLNAMES.FIRE && this.spells_learned >= 1) {
        ...
        if (this.fire_cooldown_timer <= 0) {
          ...
          new Fireball(
            this.x + this.face_direction * offset,
            this.y - offset,
            angle
          );
        ...
        this.current_spell === SPELLNAMES.EARTH &&
        ...
        this.current_spell === SPELLNAMES.AIR &&
        ...
        this.current_spell === SPELLNAMES.WATER &&
    ...
    if (this.current_spell === SPELLNAMES.WATER) {
      ...
      var water_block = IM.instancePlace(
        this,
        this.x + this.hsp,
        this.y + this.vsp + 5,
        WaterBlock
  ...
  draw(gui, camera) {
    ...
    this.animation.scale.x =
      this.facing * (this.spr_scale - Math.abs(this.vsp) / 50);
    this.animation.scale.y = this.spr_scale + Math.abs(this.vsp) / 50;
    if (Math.abs(this.hsp) > 0.1) {
      this.animation.scale.x =
        Math.abs(this.animation.scale.x) * Math.sign(this.hsp);
    }
  ...
  stepGrounded() {
    ...
    if (inp !== 0) {
      this.hsp = EngineUtils.clamp(
        this.hsp + inp * this.ground_accel,
        -this.max_run_speed,
        this.max_run_speed
      );
      // Running clouds
      if (
        Math.abs(this.hsp) > this.max_run_speed / 5 &&
        this.state_timer % 15 === 0
      ) {
        new DustParticle(
          this.x - part_from_center * Math.sign(this.hsp),
          this.y - part_from_ground,
          0.8
        );
      }
    } else {
  ...
  stepAirborne() {
    ...
    if (inp !== 0) {
      ...
      if (
        this.vsp > 0 &&
        this.collisionCheck(this.x + inp, this.y) &&
        !(coll instanceof NoWallClingWall)
      ) {
    ...
    if (!IN.keyCheck("KeyS")) {
      if (this.vsp >= 0) {
        ...
        for (const obj of collided_list) {
          if (
            obj.getHitbox().getBoundingBoxTop() >
            this.getHitbox().getBoundingBoxBottom()
          ) {
            return true;
          }
        }
  ...
  collisionCheckY() {
    ...
    if (this.vsp != 0 && this.collisionCheck(this.x, this.y + this.vsp)) {
      while (
        !this.collisionCheck(this.x, this.y + Math.sign(this.vsp)) &&
        t < Math.abs(this.vsp)
      ) {
        this.y += Math.sign(this.vsp);
        t++;
  ...
  collisionCheckX() {
    ...
    if (this.hsp != 0 && this.collisionCheck(this.x + this.hsp, this.y)) {
      while (
        !this.collisionCheck(this.x + Math.sign(this.hsp), this.y) &&
        t < Math.abs(this.hsp)
      ) {
        this.x += Math.sign(this.hsp);
        t++;
